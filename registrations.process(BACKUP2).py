...
for row in rows:
    if row[1] == 'PRCS':
        ...
        # for existing studet, determine the group membership from the directory
        group_info = ''
        ...
        if row[2] == 'Online':
            ...
            online_records.append([
                ...
                GNPS_ORG_PREFIX + str(row[1]).upper() + '/' + str(level_group.rsplit("_")[0] + '-' + level_group.rsplit("_")[1]).upper() if row[3]=='No' else GNPS_ORG_PREFIX + str(row[2]).upper() + '/' + str(group_info.rsplit("@")[0].rsplit("_")[0]).upper() + '-' + str(group_info.rsplit("@")[0].rsplit("_")[1]).upper() if group_info !='' else '', # Org Path
                ])
        else:
            # if res != None :
            #     print(str(res['primaryEmail']))
